...
enforcement) to [`sandboxes-network-credentials.md`](sandboxes-network-credentials.md), and the sandboxes a
kit is composed into to [`sandboxes-lifecycle.md`](sandboxes-lifecycle.md).
...
- The task is creating/running/removing the sandbox a kit is composed into,
  independent of the kit's own content — use [`sandboxes-lifecycle.md`](sandboxes-lifecycle.md).
- The task is what a credential or network rule a kit declares actually
  ...
  [`sandboxes-network-credentials.md`](sandboxes-network-credentials.md).
- The task is the `sbxenv.yaml` file format that references kits via its
  own `kits:` block — use [`sandboxes-env.md`](sandboxes-env.md) for that file's schema
...
- Do not redefine a base agent's credential in a mixin: declaring a new
  ...
  on sandbox kits, never mixins. See `sandboxes-kits/spec-v2-fields.md`.
...
- `image:` is the pre-built base image. `entrypoint:` is the fixed process
  ...
  For a complete minimal kit, use `../assets/sandboxes-kits/spec-sandbox.yaml`, which inherits
...
- **`permissions.network.allow` is additive across a composition, and a
  ...
  independently of any kit) permits — see [`sandboxes-network-credentials.md`](sandboxes-network-credentials.md). **Removing a host from one kit's `allow` list does not by
...
- `apiKey.inject[]` needs a `domain` and either an explicit `header`+
  ...
  `sandboxes-kits/spec-v2-fields.md` for a complete fragment.
...
Optional fragment for the shell kit in `../assets/sandboxes-kits/spec-sandbox.yaml`:
```yaml
setup:
  startup:
...
See `sandboxes-kits/kit-distribution-commands.md` for full flag lists and
worked examples of each command above.

## Related skills

- For the sandboxes a kit is composed into (`sbx create`/`run --kit`,
  `sbx kit add SANDBOX`), use [`sandboxes-lifecycle.md`](sandboxes-lifecycle.md).
...
- For the `sbxenv.yaml` file whose `kits:`/`agent:` fields reference a kit
  by this schema, use [`sandboxes-env.md`](sandboxes-env.md).

## References

- `sandboxes-kits/sources.md` — provenance for every rule above (spec package, SPEC-v2.md, help captures, docs URLs).
- `sandboxes-kits/spec-v2-fields.md` — the complete v2 field table (common fields, sandbox-only fields, mixin-only fields, shared blocks) for lookup without re-reading the full spec.
- `sandboxes-kits/kit-distribution-commands.md` — full flags and worked examples for `sbx kit validate/inspect/pack/pull/push/provenance/sign/verify/add`.

## Assets

- `../assets/sandboxes-kits/spec-sandbox.yaml` — a genuine minimal `kind: sandbox` kit that
  `extends: shell` to inherit a real, working image rather than inventing
  one.
- `../assets/sandboxes-kits/spec-mixin.yaml` — a genuine minimal `kind: mixin` kit with no
  credentials at all (an egress-only extension), which composes cleanly
  with every built-in agent.

## Checks

- `../checks/sandboxes-kits.md` — Schema, composition, egress, and kit-add checks (unexecuted integration runbook; isolated `--app-name`, no registry publishing or signing).
