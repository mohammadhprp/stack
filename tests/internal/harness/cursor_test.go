...
func TestCursorPlanSkills(t *testing.T) {
	adapter, _ := harness.Get("cursor")
...
func TestCursorPlanMCPsPathUnchanged(t *testing.T) {
	adapter, _ := harness.Get("cursor")
	...
	if len(files) != 1 || files[0].Path != ".cursor/mcp.json" {
		t.Fatalf("cursor MCP path changed: %+v", files)
	}
}
