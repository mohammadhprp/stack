...
1. **`.dockerignore`** — Create this first so the initial build context is small and safe. See `../assets/project-foundations/dockerignore-example` for a reference.
2. **`Dockerfile`** — Create a working starter image definition that the project can build and run with. See `../assets/project-foundations/Dockerfile.simple`.
3. **`compose.yaml`** — Create a local development stack that includes the application service and any required dependencies. See `../assets/project-foundations/compose-dev.yaml`.
...
- For Dockerfile optimization, cache strategy, non-root execution, and image hardening, use [`build-strategies.md`](build-strategies.md).
- For service dependencies, health checks, overrides, volumes, networks, and Compose debugging, use [`compose-patterns.md`](compose-patterns.md).
...
- `project-foundations/project-structure.md` — Detailed guidance on Docker project file organization, naming conventions, and multi-service layouts.

## Assets

- `../assets/project-foundations/dockerignore-example` — A comprehensive `.dockerignore` for a typical project.
- `../assets/project-foundations/compose-dev.yaml` — A development-oriented Compose file with Dockerized dependencies.
- `../assets/project-foundations/Dockerfile.simple` — A basic multi-stage Dockerfile following best practices.

## Scripts

- **`scripts/project-foundations/verify-setup.sh`** — Checks required files exist and validates `compose.yaml`.
  ```bash
  bash scripts/project-foundations/verify-setup.sh [--help]
  ```
...
- `../checks/project-foundations.md` — Detailed verification checklist for manual review.
