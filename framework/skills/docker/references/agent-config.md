...
Do not use this skill when:
- The task is about running the CLI (`docker agent run` flags, `--safety`, `--sandbox`, aliases, worktrees) — use [`agent-run.md`](agent-run.md).
- The task is about exposing an agent as a server (`serve mcp/api/a2a/acp/chat`), distributing it (`share push/pull`), or evaluating it (`docker agent eval`) — use [`agent-deploy.md`](agent-deploy.md).
- The task is about a generic Dockerfile or Compose service unrelated to Docker Agent — use [`build-strategies.md`](build-strategies.md) or [`compose-patterns.md`](compose-patterns.md).
...
- For external tools, prefer an MCP server from Docker's MCP catalog over a
  ...
  `agent-config/toolsets-and-providers.md`.
...
- A coordinator delegates via `sub_agents: [name, ...]`; listing sub-agents
  ...
  ```
  Use `../assets/agent-config/team-agent.yaml` for the complete runnable team, including
...
## Related skills
- For running the agent (`docker agent run`, safety modes, sandbox, aliases), use [`agent-run.md`](agent-run.md).
- For serving, sharing, or evaluating the agent, use [`agent-deploy.md`](agent-deploy.md).

## References
- `agent-config/toolsets-and-providers.md` — full built-in toolset list, MCP connection modes, and the provider/env-var table.
- `agent-config/sources.md` — provenance of every rule in this skill.

## Assets
- `../assets/agent-config/team-agent.yaml` — a runnable multi-agent team template (coordinator + coder + reviewer).

## Checks
- Before running an agent, follow `../checks/agent-config.md` to confirm its
