...
It assumes the agent config already exists — see [`agent-config.md`](agent-config.md) for
authoring it, and [`agent-run.md`](agent-run.md) for interactive/local invocation.
...
Do not use this skill when:
- The task is authoring the agent.yaml itself (models, toolsets, sub_agents) — use [`agent-config.md`](agent-config.md).
- The task is running the agent interactively on a developer's machine, choosing `--safety`/`--sandbox`, or aliases — use [`agent-run.md`](agent-run.md).
...
- Four scoring dimensions: Tool Calls (F1 against the recorded sequence),
  ...
  checks; see the complete assertion-type list in `agent-deploy/eval-format.md`).
...
## Related skills
- For writing or changing the underlying `agent.yaml`, use [`agent-config.md`](agent-config.md).
- For local/interactive runs, safety-mode choice, and sandboxing, use [`agent-run.md`](agent-run.md).

## References
- `agent-deploy/eval-format.md` — full eval session JSON schema and CLI flag table.
- `agent-deploy/sources.md` — provenance of every rule in this skill.

## Assets
- `../assets/agent-deploy/eval-session-example.json` — a minimal eval session file to copy and adapt.

## Checks
- `../checks/agent-deploy.md` — Verification runbook for serving, sharing, and evaluating an agent.
