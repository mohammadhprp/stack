...
- The task uses standalone `sbx run/create/stop/rm` rather than
  `docker agent run --sandbox` — use [`sandboxes-lifecycle.md`](sandboxes-lifecycle.md).
- The task is standalone `sbx policy` or `sbx secret` configuration — use
  [`sandboxes-network-credentials.md`](sandboxes-network-credentials.md). Establish which CLI is in use
  before recommending commands when the request only says "my sandbox".
- The task is writing or editing the `agent.yaml` itself (models, toolsets, sub_agents) — use [`agent-config.md`](agent-config.md).
- The task is exposing an agent as a server (`serve`), sharing it via a registry (`share`), or evaluating it (`eval`) — use [`agent-deploy.md`](agent-deploy.md).
...
- An agent that only *describes* a plan instead of executing it is usually
  ...
  `agent.yaml`), not a model failure — hand this back to [`agent-config.md`](agent-config.md).
...
## Related skills
- For standalone `sbx` lifecycle commands, use [`sandboxes-lifecycle.md`](sandboxes-lifecycle.md).
- For standalone `sbx policy` and `sbx secret`, use [`sandboxes-network-credentials.md`](sandboxes-network-credentials.md).
- For writing or changing the underlying `agent.yaml` (models, toolsets, sub_agents), use [`agent-config.md`](agent-config.md).
- For serving, sharing, or evaluating the agent, use [`agent-deploy.md`](agent-deploy.md).

## References
- `agent-run/safety-and-sandbox.md` — full safety-mode/flag interaction table and sandbox trust-boundary details.
- `agent-run/sources.md` — provenance of every rule in this skill.

## Assets
- None.

## Checks
- `../checks/agent-run.md` — Verification runbook for a `docker agent run` invocation.
