...
- The task is running `docker agent run --sandbox` or managing its
  `docker agent sandbox` allowlist — use [`agent-run.md`](agent-run.md). If the CLI
...
- The task is about what a sandbox can reach on the network or which
  credentials it uses — use [`sandboxes-network-credentials.md`](sandboxes-network-credentials.md).
- The task is authoring or running a declarative `sbxenv.yaml` file — use
  [`sandboxes-env.md`](sandboxes-env.md).
- The task is authoring, packaging, signing, or composing a kit `spec.yaml`
  — use [`sandboxes-kits.md`](sandboxes-kits.md).
...
- Additional workspaces are extra positional paths after the first. Append
  ...
  **Never mount a secrets/credentials file this way** (`:ro` or otherwise) —
  ...
  instead; see [`sandboxes-network-credentials.md`](sandboxes-network-credentials.md).
...
- For declarative, checked-in `sbxenv.yaml` environments that wrap this same
  create/run/rm lifecycle, use [`sandboxes-env.md`](sandboxes-env.md).
- For authoring or composing the kit `spec.yaml` an `AGENT` reference can
  point to, use [`sandboxes-kits.md`](sandboxes-kits.md).

## References

- `sandboxes-lifecycle/sources.md` — provenance for every rule above (help captures, source paths, docs URLs).
...
- `../checks/sandboxes-lifecycle.md` — Verification runbook for sandbox lifecycle commands (unexecuted runbook; run manually with an isolated `--app-name`, never with `--force` except consented cleanup of the runbook's own uniquely-named test sandboxes).
