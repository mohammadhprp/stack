...
- For first-time Docker project scaffolding and baseline file creation, use [`project-foundations.md`](project-foundations.md).
- For Dockerfile internals, build caching, multi-stage builds, and `.dockerignore`, use [`build-strategies.md`](build-strategies.md).
- For destructive Docker CLI commands outside Compose (`docker system prune`, `docker rm -f`, image/network/builder pruning, standalone volume deletion) and a cross-product index of destructive-command guardrails, use [`destructive-guardrails.md`](destructive-guardrails.md).

## References

- `compose-patterns/service-dependencies.md` — Detailed guidance on `depends_on`, health check patterns for common databases, and startup ordering strategies.
- `compose-patterns/volumes-and-networks.md` — Patterns for volume mounts, named volumes, bind mounts, and network configuration.

## Assets

- `../assets/compose-patterns/compose-web-app.yaml` — Complete multi-service web app (app + Postgres + Redis) with health checks, dependencies, and named volumes.
- `../assets/compose-patterns/compose-dev-override.yaml` — Development override showing bind mounts, debug ports, and Compose Watch configuration.
- `../assets/compose-patterns/bad-vs-good.md` — Before/after comparisons of common Compose mistakes and their fixes.

## Scripts

- **`scripts/compose-patterns/verify-compose.sh`** — Validates `compose.yaml` with `docker compose config --quiet`, without printing resolved configuration.
  ```bash
  bash scripts/compose-patterns/verify-compose.sh [--help]
  ```
...
- `../checks/compose-patterns.md` — Detailed verification runbook for manual review.
