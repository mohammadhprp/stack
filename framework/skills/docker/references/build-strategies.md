...
See `build-strategies/multi-stage-builds.md` for language-specific patterns (Go, Node, Python, Java).
...
See `build-strategies/layer-caching.md` for detailed cache invalidation rules and cache mount patterns.
...
See `build-strategies/multi-stage-builds.md` for per-language patterns (npm, pip, Maven, Go `GOPRIVATE`).
...
See `../assets/build-strategies/dockerignore-example` for a comprehensive template.
...
- For first-time Docker project scaffolding and deciding which files to create, use [`project-foundations.md`](project-foundations.md).
- For service dependencies, health checks, overrides, networks, and volume patterns, use [`compose-patterns.md`](compose-patterns.md).
- For destructive Docker CLI commands (`docker system prune`, `docker rm -f`, image/network/builder pruning) and a cross-product index of destructive-command guardrails, use [`destructive-guardrails.md`](destructive-guardrails.md).

## References

- `build-strategies/multi-stage-builds.md` — Language-specific multi-stage patterns for Go, Node.js, Python, and Java
- `build-strategies/layer-caching.md` — Deep dive on layer ordering, cache invalidation, and BuildKit cache mounts

## Assets

- `../assets/build-strategies/Dockerfile.golang` — Multi-stage Go build with distroless runtime and non-root user
- `../assets/build-strategies/Dockerfile.nodejs` — Multi-stage Node.js build with proper layer caching and non-root user
- `../assets/build-strategies/Dockerfile.python` — Python build with virtual env, layer ordering, and non-root user
- `../assets/build-strategies/dockerignore-example` — Comprehensive `.dockerignore` template

## Scripts

- **`scripts/build-strategies/verify-build.sh`** — Builds the image, reports size and configured user.
  ```bash
  bash scripts/build-strategies/verify-build.sh [--help] [IMAGE_NAME]
  ```
...
- `../checks/build-strategies.md` — Detailed verification runbook for manual review.
