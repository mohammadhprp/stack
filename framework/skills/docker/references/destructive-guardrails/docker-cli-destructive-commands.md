...
Detailed breakdown of the generic Docker CLI commands this reference owns. For each command: what it deletes, the exact flags that widen the blast radius, and the safer/scoped alternative to reach for first.
...
These four commands remove or kill containers, but each widens the blast radius differently. See `../destructive-guardrails.md`'s Core guidance for the Tier 1/Tier 2 confirmation policy that applies to each — this section covers the exact flag behavior only.
...
- Does not remove the container — it can be restarted afterward with `docker start`. This is why it sits outside the Tier 1/Tier 2 removal model in `../destructive-guardrails.md`'s Core guidance, even though Docker Agent's runtime safety classifier still flags it as low-risk destructive.
...
Standalone case only — no Compose project in play. If a `compose.yaml` is present and the volume belongs to that project, use [`compose-patterns.md`](../compose-patterns.md) instead (`docker compose down -v`, Compose-managed anonymous volumes); this section does not duplicate or override that guidance.
...
Standalone case only — no Compose project in play. For Compose-managed anonymous volumes, use [`compose-patterns.md`](../compose-patterns.md) (`docker compose down -v`) instead.
