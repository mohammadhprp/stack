...
- The task is declaring secrets/registries/bindings inside a checked-in
  `sbxenv.yaml` file — use [`sandboxes-env.md`](sandboxes-env.md) for the file format (this
...
- The task is declaring a kit's own `credentials:`/`permissions.network:`
  block in a `spec.yaml` — use [`sandboxes-kits.md`](sandboxes-kits.md) for the schema (this
...
- For creating, reattaching to, and removing the sandboxes these policies
  and secrets apply to, use [`sandboxes-lifecycle.md`](sandboxes-lifecycle.md).
...
- `sandboxes-network-credentials/sources.md` — provenance for every rule above (help captures, source paths, docs URLs).
...
- `../checks/sandboxes-network-credentials.md` — Verification runbook for network policy and secret commands (unexecuted runbook; run manually with an isolated `--app-name`, no real secret values).
